...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    greeting_message = (
        f"👋 Hello {user_name}!\n\n"
        "🚀 **Enhanced Credential Extractor Bot v2.0** 🤖\n\n"
        "✨ **New Features:**\n"
...
async def show_enhanced_queue_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    message = "📊 **Enhanced Processing Queue Status**\n\n"
    
    if status['currently_processing_count'] > 0:
        message += f"🔄 **Currently processing:** {status['currently_processing_count']} files\n"
    ...
    if user_files:
        message += f"📁 **Your files ({len(user_files)}):**\n"
