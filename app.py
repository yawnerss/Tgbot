...
# Add a scheduler guard and an ensure function:
SCHEDULER_STARTED = False

def ensure_scheduler_started():
  """
  Start the background scheduler exactly once per process.
  Note: If your deployment runs multiple workers, each worker will start one thread.
  Keep 1 worker or use a distributed lock to avoid duplicates.
  """
  global SCHEDULER_STARTED
  if not SCHEDULER_STARTED:
      start_scheduler()
      SCHEDULER_STARTED = True
      print("⏱️ Scheduler started via ensure_scheduler_started()")
...
def scheduler_loop():
  ...
  while True:
      ...
      if items:
          print(f"🔎 Scheduler cycle: checking {len(items)} subscription(s)")
      ...
      for user_id, sub in items:
          ...
          if now - last_sent >= interval:
              print(f"📨 Sending scheduled update to {user_id} for '{location}'")
              weather = get_weather(location)
              send_message(user_id, f"⏰ 5-min update for {location}:\n\n{weather}")
              with subscriptions_lock:
                  if user_id in subscriptions:
                      subscriptions[user_id]["last_sent"] = now
              save_subscriptions()

      time.sleep(10)  # was 30; faster while testing
...
# Add two small debug endpoints to verify the scheduler/subscriptions and trigger a test cycle:
...
def admin_trigger():
  """
  Manually run one scheduler pass for quick testing.
  """
  now = time.time()
  sent = 0
  with subscriptions_lock:
  ...
  for user_id, sub in items:
      ...
      if not location:
          continue
      weather = get_weather(location)
      send_message(user_id, f"🧪 Manual trigger for {location}:\n\n{weather}")
      with subscriptions_lock:
          if user_id in subscriptions:
              subscriptions[user_id]["last_sent"] = now
      save_subscriptions()
      return jsonify({"ok": True, "checked": len(items)}), 200
  ...
  try:
      ...
      if msg_lower.startswith("set ") or msg_lower.startswith("subscribe ") or msg_lower.startswith("track "):
          ...
          if len(parts) > 1 and parts[1].strip():
              ...
              # 1) Immediate weather fetch so users see it now
              ...
              # 2) Schedule next update in 5 minutes
              with subscriptions_lock:
                  if sender_id in subscriptions:
                      subscriptions[sender_id]["last_sent"] = time.time()
              save_subscriptions()
          else:
